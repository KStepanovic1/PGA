...
def main(
    ...
    x_train, x_test, y_train, y_test, scaler = train_test_dataset(x=x, y=y)
    plt.plot(x_test, y_test, ".", color="blue", label="Real data")
    colors = ["green", "cyan"]
    """
    alpha_ = [0.7, 0.15, 0]
    colors = ["#990000", "#FF0000", "#FF9999"]
    labels = [
        r"$\frac{1}{N}\sum_{i=1}^N |y_i-\hat{y}_i|+\frac{\alpha}{N}\sum_{i=1}^N max(0,\hat{y}_i-y_{min})$",
        r"$\frac{1}{N}\sum_{i=1}^N (y_i-\hat{y}_i)^2+\frac{\alpha}{N}\sum_{i=1}^N (\hat{y}_i-y_{min})^2$",
        r"$\frac{1}{N}\sum_{i=1}^N w_i (y_i-\hat{y}_i)^2$",
    ]
    """
    ...
    y_pred, y_pred_train = prediction(
        x_train=x_train, x_test=x_test, model=model, scaler=scaler
    )
    plt.plot(x_test, y_pred, ".", label="Predicted data", color="#FF0000")
    time_gd_ = []
    for i, starting_point in enumerate(starting_points):
        ...
        plt.plot(
            ...
            scaler.inverse_transform(
                np.array(np.array(model(np.array([starting_point])))[0][0]).reshape(
                    -1, 1
                )
            ),
            marker="v",
    ...
    y_lp = scaler.inverse_transform(np.array(y_lp).reshape(-1, 1))
...
def train_test_dataset(x, y):
    ...
    # scale the output dataset between 0 and 1
...
def train_nn(x_train, feature_num, y_train, layer_size, batch_size, epochs, alpha, i):
    """
    ...
    model.compile(optimizer=opt, loss=custom_loss(alpha=alpha, y_min=y_min, i=i))
    ...
    model.save("model_non_convex_icnn.h5")
...
def extract_weights():
    ...
    """
    model = load_model("model_non_convex_icnn.h5", compile=False)
    model.summary()
    theta = {}
    j = 0
...
def prediction(x_train, x_test, model, scaler):
    """
    Predicts the output based on the input and the model
    """
    y_pred_model = model.predict(x_test)
    # rescale back the predicted data (using the scaler from training dataset)
    y_pred = y_pred_model.reshape(-1, 1)
...
def optimize_input_via_lp(layer_size, lower_bound, upper_bound, model_nn):
    ...
    start_time = time.time()
    model = Model("non_convex_lp_icnn_inference")
    model.reset()
