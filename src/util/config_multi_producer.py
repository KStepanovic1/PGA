from pathlib import Path

id = "multiproducer01"
# =========================================================
# File Path
# =========================================================
Dataset = {
    "Path": (Path(__file__).parents[2] / "data"),
    "FileName": "processed_data.csv",
    "train_test_split_point": 0.8,
}

Paths = {
    "Results": (Path(__file__).parents[2] / "results"),
    "Plots": (Path(__file__).parents[2] / "plots"),
}

# =========================================================
# Global Parameters
# =========================================================
PhysicalProperties = {
    "Density": 963,
    "HeatCapacity": 4181.3,
    "MaxTemp": 120,
    "MinSupTemp": 70,
    "EnergyUnitConversion": 10 ** 6,  # The scale to convert given energy unit to Watt
}

GridProperties = {"ConsumerNum": 1}

GridProperties["PipeNum"] = (
    2 * GridProperties["ConsumerNum"]
    if GridProperties["ConsumerNum"] == 1
    else 2 * (GridProperties["ConsumerNum"] + 1)
)

TimeParameters = {
    "TimeInterval": 3600,  # seconds
    "PlanningHorizon": 24,  # steps
    "ActionHorizon": 1,  # steps
}

# =========================================================
# Grid Parameters
# =========================================================
# combined-cycle extraction-condensation CHP
CHPPreset1 = {
    "CHPType": "keypts",
    "OperationRegion": [[0,0], [0, 86.67], [50, 75]],  # Heat, power. MW
    "Efficiency": 1,
    "FuelCost": [0.449*20, 1.923*20],
    "MaxRampRateQ": -1,
    "MaxRampRateE": -1,
    "MaxRampRateTemp": 3,
    "StartupCost":0,
    "InitialStatus":True,
}

# back-pressure steam turbine
CHPPreset2 = {
    "CHPType": "keypts",
    "OperationRegion": [[0,0], [0,0.01], [30,15.01], [30,15]],  # Heat, power. MW
    "Efficiency": 1,
    "FuelCost": [1.11*20, 1.11*20],
    "MaxRampRateQ": -1,
    "MaxRampRateE": -1,
    "MaxRampRateTemp": 3,
    "StartupCost":0,
    "InitialStatus":True,
}

# gas boiler
CHPPreset3 = {
    "CHPType": "keypts",
    "OperationRegion": [[0,0], [0,0.01], [10,0.01], [10,0]],  # Heat, power. MW
    "Efficiency": 1,
    "FuelCost": [0.95*20, 0.95*20],
    "MaxRampRateQ": -1,
    "MaxRampRateE": -1,
    "MaxRampRateTemp": 3,
    "StartupCost":0,
    "InitialStatus":True,
}

# electricity boiler
CHPPreset4 = {
    "CHPType": "keypts",
    "OperationRegion": [[0,-0.01], [0,0], [15,-14], [15,-14.01]],  # Heat, power. MW
    "Efficiency": 1,
    "FuelCost": [0.0, 0.0],
    "MaxRampRateQ": -1,
    "MaxRampRateE": -1,
    "MaxRampRateTemp": 3,
    "StartupCost":0,
    "InitialStatus":True,
}

ProducerPreset1 = {
    "Generators": [CHPPreset1, CHPPreset2, CHPPreset3, CHPPreset4],
    "PumpEfficiency": 1,
    "ControlWithTemp": True,
}


ConsumerPreset1 = {
    "k": 5 * 10 ** 6 / 400 * (400 ** (-0.8) + 400 ** (-0.8)),
    "q": 0.8,
    "SurfaceArea": 400,
    "TempReturnSeconary": 45,
    "SetPointTempSupplySecondary": 70,
    "MaxMassFlowPrimary": 805.15,
    "MinTempSupplyPrimary": 70,
    "FixPressureLoad": 100000,
}

PipePreset1 = {
    "Diameter": 0.5958,
    "Length": 12000,
    "ThermalResistance": 1.36,  # in k*m/W, = 1/U
    "InitialTemperature": 90,
    "EnvironmentTemperature": 10,
    "MaxFlowSpeed": 3,
    "MinFlowSpeed": 0,
    "FrictionCoefficient": 1.29 * 1.414,
}

PipePreset2 = {
    "Diameter": 0.5958,
    "Length": 12000,
    "ThermalResistance": 1.36,  # in k*m/W, = 1/U
    "InitialTemperature": 50,
    "EnvironmentTemperature": 10,
    "MaxFlowSpeed": 3,
    "MinFlowSpeed": 0,
    "FrictionCoefficient": 1.29 * 1.414,
}

# supply side edge
PipePreset3 = {
    "Diameter": 0.2,
    "Length": 3000,
    "ThermalResistance": 1.1,  # in k*m/W, = 1/U
    "InitialTemperature": 90,
    "EnvironmentTemperature": 10,
    "MaxFlowSpeed": 3,
    "MinFlowSpeed": 0,
    "FrictionCoefficient": 1.29 * 2,
}

# return side edge
PipePreset4 = {
    "Diameter": 0.2,
    "Length": 3000,
    "ThermalResistance": 1.1,  # in k*m/W, = 1/U
    "InitialTemperature": 50,
    "EnvironmentTemperature": 10,
    "MaxFlowSpeed": 3,
    "MinFlowSpeed": 0,
    "FrictionCoefficient": 1.29 * 2,
}
# GridParameters = {
#   'Producers': [
#     {
#       'Type': 'CHP',
#       'Parameters': CHPPreset1,
#       'PumpEfficiency': 1,
#       'ControlWithTemp': True,
#     }
#   ],
#   'Consumers': [
#     HXPreset1,
#   ],
#   'Pipes': [
#     PipePreset1,
#     PipePreset2,
#   ],
# }