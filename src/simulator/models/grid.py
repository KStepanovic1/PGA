...
from src.simulator.models.edge import Edge
from src.simulator.models.grid_object import GridObject
from src.simulator.models.timing import Timing
from src.simulator.models.heat_exchanger import timing as heat_exchanger_timing
from src.simulator.models.edge import timing as edge_timing
from src.simulator.interfaces.grid_interface import GridInterface
...
class Grid(GridInterface):
    ...
    def reset(
        ...
        history_mass_flow: Optional[dict] = None,  # {edge_id: ms}
        history_pressure: Optional[dict] = None,  # {node_id: pressure}
    ) -> None:
        ...
        if history_mass_flow is not None:
            for edge in self.edges:
                edge.set_historical_mass_flow(history_mass_flow[edge.id])
        self.clear()
    ...
    def get_history_flow_and_pressure(
        self,
        time_step=0,
    ):
        history_mass_flow = {}
        for edge in self.edges:
            history_mass_flow[edge.id] = edge.mass_flow[0, time_step]

        history_pressure = {}
        for node in self.nodes:
            history_pressure[node.id] = node.pressure[1, time_step]

        return history_mass_flow, history_pressure
    ...
    def debug_solve(timing: Timing):
        ...
        print(
            "..of which heat exchanger: {:.1f} sec".format(heat_exchanger_timing.get())
        )
