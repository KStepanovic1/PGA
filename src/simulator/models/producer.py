...
class Producer(Node):
    ...
    def get_outlet_temp(
        ...
        if self.control_with_temp:
            ...
            try:
                temp += self.edges[0].get_outlet_temp()[0]
            except AssertionError:
                print(
                    "Warning, when producer is controlled with q and water reaches consumer at the same time step,"
                    + "simulation will not be accurate. Try use smaller time interval or control with temp"
                )
                if self.current_step > 0:
                    ...
                    if (mass_flow is None) or np.isnan(mass_flow):
                        ...
                        temp += self.edges[0].initial_plug_cache[-1].entry_temp

        entry_step_global = (
            self.current_step
        )  # counting of entry step global starts from producer
