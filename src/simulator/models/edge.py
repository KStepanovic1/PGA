...
from src.simulator.models.producer import Producer
from src.simulator.models.grid_object import GridObject
from src.simulator.models.plug import Plug
from src.simulator.models.timing import Timing

if TYPE_CHECKING:
    from src.simulator.models.node import Node
...
class Edge(GridObject):
    def __init__(
        ...
        historical_mass_flow: Optional[float] = None,  # in kg/s
        ...
        self.historical_mass_flow = historical_mass_flow
        ...
        self.temp, self.actual_outlet_temp, self.mass_flow, self.flow_speed = (
            None,
            None,
            None,
            None,
    ...
    def clear(self) -> None:
        ...
        self.violations = defaultdict(
            lambda: np.full(self.blocks, np.nan, dtype=float)
        )
    ...
    def get_outlet_temp(self) -> float:
        ...
        previous_mass_flow = (
            self.mass_flow[0, self.current_step - 1]
            if self.current_step > 0
            else self.historical_mass_flow
        )

        if self.current_step == 0:
            if self.historical_mass_flow is not None:
                expected_mass = self.interval_length * abs(self.historical_mass_flow)
            else:
                expected_mass = 0
        else:
            expected_mass = (
        ...
        for plug in reversed(self.plug_cache):
            ...
            plug_temp = self.get_plug_temp(
                self.current_step - plug.entry_step, plug.entry_temp
            )
    ...
    def get_outlet_temp_mass_bundle(self):
        ...
        if not issubclass(type(inlet_node), Producer):
            ...
            if self.current_step > 0:
                ...
                entry_temp, e_s_g = inlet_node.get_outlet_temp(
                    inlet_slot, self.historical_mass_flow
                )
    ...
    def set_mass_flow(self, slot: int, mass_flow: float) -> None:
        ...
        if not issubclass(type(inlet_node), Producer):
            ...
            entry_temp, entry_step_global = inlet_node.get_outlet_temp(
                inlet_slot, mass_flow
            )
        """
        ...
        actual_outlet_temp, delay_arr, entry_step_global = self.push_plugs_outside(
            consumed_mass
        )
    ...
    def set_historical_mass_flow(self, mass_flow):
        self.historical_mass_flow = mass_flow
