...
# consumer is a secondary grid, with about 250 households, see de Keijzer p4
consumer_demand = np.array(
    [
        1200 * 10 ** 3,  # peak, see de Keijzer p23
        1200 * 10 ** 3,  # peak, see de Keijzer p23
        1200 * 10 ** 3,  # peak, see de Keijzer p23
        1200 * 10 ** 3,  # peak, see de Keijzer p23
        1200 * 10 ** 3,  # peak, see de Keijzer p23
        600 * 10 ** 3,  # peak, see de Keijzer p23
        600 * 10 ** 3,  # peak, see de Keijzer p23
        600 * 10 ** 3,  # peak, see de Keijzer p23
        600 * 10 ** 3,  # peak, see de Keijzer p23
        600 * 10 ** 3,  # peak, see de Keijzer p23
...
* area approx 0.03 m2
* velocity max 2.0 m/s (de Keijzer p24 and Valkema p24)
* 963kg/m^3
=> 62.2 kg/s
...
for i in range(0, hts_count):
    supply_edge = Edge(
        blocks=blocks,
        historical_t_in=90,
        diameter=0.5958,  # typically DN600 in Utrecht, see de Keijzer p53
        length=2704,
    ...
    for j in range(0, consumers_per_hts):
        c_supply_edge = Edge(
            blocks=blocks,
            historical_t_in=85,
            diameter=0.203,  # typically DN200 in Utrecht, see de Keijzer p53
            length=2000,
